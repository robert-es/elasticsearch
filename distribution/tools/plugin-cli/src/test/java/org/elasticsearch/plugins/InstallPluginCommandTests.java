...
public class InstallPluginCommandTests extends ESTestCase {
    ...
    public static void testIfFipsMode() {
        assumeFalse("Can't run in a FIPS JVM because this depends on BouncyCastle (non-fips)",
            inFipsJvm());
    }
    ...
    public void setUp() throws Exception {
        ...
        skipJarHellCommand = new InstallPluginCommand() {
            @Override
            void jarHellCheck(PluginInfo candidateInfo, Path candidate, Path pluginsDir,
                Path modulesDir) throws Exception {
                // no jarhell check
            }
        };
    ...
    public static Iterable<Object[]> parameters() {
        ...
        parameters.add(
            new Parameter(PathUtils.getDefaultFileSystem(), LuceneTestCase::createTempDir));
        return parameters.stream().map(p -> new Object[]{ p.fileSystem, p.temp }).collect(
            Collectors.toList());
    }

    private static Configuration toPosix(Configuration configuration) {
        return configuration.toBuilder().setAttributeViews("basic", "owner", "posix", "unix")
            .build();
    }
        ...
        try (ZipOutputStream stream = new ZipOutputStream(Files.newOutputStream(jar))) {
            for (String clazz : classes) {
                stream.putNextEntry(new ZipEntry(
                    clazz + ".class")); // no package names, just support simple classes
            }
        }
    }
        ...
        try (ZipOutputStream stream = new ZipOutputStream(Files.newOutputStream(zip))) {
            Files.walkFileTree(structure, new SimpleFileVisitor<Path>() {
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
                    throws IOException {
                    String target = (prefix == null ? "" : prefix + "/") + structure.relativize(
                        file).toString();
    ...
    static String createPluginUrl(String name, Path structure, String... additionalProps)
        throws IOException {
        return createPlugin(name, structure, additionalProps).toUri().toURL().toString();
    }

    static void writePlugin(String name, Path structure, String... additionalProps)
        throws IOException {
    ...
    static void writePluginSecurityPolicy(Path pluginDir, String... permissions)
        throws IOException {
        ...
        securityPolicyContent.append("\n};\n");
        Files.write(pluginDir.resolve("plugin-security.policy"),
            securityPolicyContent.toString().getBytes(StandardCharsets.UTF_8));
    }

    static Path createPlugin(String name, Path structure, String... additionalProps)
        throws IOException {
    ...
    void installPlugin(String pluginUrl, Path home, InstallPluginCommand command) throws Exception {
        Environment env = TestEnvironment.newEnvironment(
            Settings.builder().put("path.home", home).build());
...
//                        assertEquals(InstallPluginCommand.BIN_FILES_PERMS,
...
//
//                PosixFileAttributes attributes = Files.getFileAttributeView(configDir,
...
//            () -> installPlugin("://host:1234", env.v1()));
//        assertTrue(e.getMessage(), e.getMessage().contains("no protocol"));
//    }
//
//    public void testFileNotMaven() throws Exception {
//        Tuple<Path, Environment> env = createEnv(fs, temp);
//        String dir = randomAlphaOfLength(10) + ":" + randomAlphaOfLength(5) + "\\" +
...
//        Tuple<Path, Environment> env = createEnv(fs, temp);
//        Path pluginDir = createPluginDir(temp);
//        try (PosixPermissionsResetter pluginsAttrs = new PosixPermissionsResetter(
//            env.v2().pluginsFile())) {
...
//        String pluginZip = createPluginUrl("fake",
//            pluginDirectory); // adds plugin.jar with FakePlugin
...
//    }
    ...
    public void testJarHell2() throws Exception {
        // jar hell test needs a real filesystem
        assumeTrue("real filesystem", isReal);
        Tuple<Path, Environment> environment = createEnv(fs, temp);
        String pluginZip =
            "file:/Users/a123/workspace/qunhe/java/tag-match/target/releases/tag-match-6.7.0.zip";
        IllegalStateException e = expectThrows(IllegalStateException.class,
            () -> installPlugin(pluginZip, environment.v1(), defaultCommand));
        assertTrue(e.getMessage(), e.getMessage().contains("jar hell"));
        assertInstallCleaned(environment.v2());
...
//            e.getMessage().contains("Directories not allowed in bin dir for plugin"));
//        assertInstallCleaned(env.v2());
//    }
//
...
//            () -> installPlugin(pluginZip, env.v1()));
//        assertTrue(e.getMessage(),
//            e.getMessage().contains(env.v2().binFile().resolve("elasticsearch").toString()));
//        assertInstallCleaned(env.v2());
//    }
//
...
//        Files.write(envConfigDir.resolve("custom.yml"),
...
//        installPlugin(pluginZip, env.v1());
//        assertPlugin("fake", pluginDir, env.v2());
//        List<String> configLines = Files.readAllLines(envConfigDir.resolve("custom.yml"),
...
//            () -> installPlugin(pluginZip, env.v1()));
//        assertTrue(e.getMessage(),
//            e.getMessage().contains("Directories not allowed in config dir for plugin"));
//        assertInstallCleaned(env.v2());
//    }
//
...
//            containsString("This plugin was built with an older plugin structure"));
//        assertInstallCleaned(env.v2());
//    }
//
...
//            () -> installPlugin(pluginZip, env.v1()));
//        assertTrue(e.getMessage(),
//            e.getMessage().contains("resolving outside of plugin directory"));
//        assertInstallCleaned(env.v2());
//    }
//
...
//        runInstallXPackTest(Build.Flavor.DEFAULT, UserException.class,
...
//        runInstallXPackTest(Build.Flavor.UNKNOWN, IllegalStateException.class,
//            "your distribution is broken");
//    }
//
//    private <T extends Exception> void runInstallXPackTest(
//        final Build.Flavor flavor, final Class<T> clazz, final String expectedMessage)
//        throws IOException {
...
//        final T exception = expectThrows(clazz,
//            () -> flavorCommand.execute(terminal, "x-pack", false, environment));
...
//
//        UserException e = expectThrows(UserException.class,
//            () -> installPlugin("analysis-smartnc", env.v1()));
//        assertThat(e.getMessage(),
...
//            containsString("WARNING: plugin requires additional permissions"));
...
//        final UserException e = expectThrows(UserException.class,
//            () -> installPlugin(pluginZip, env.v1(),
//                randomFrom(skipJarHellCommand, defaultCommand)));
//        assertThat(
//            e.getMessage(),
//            equalTo("plugin directory [" + env.v2().pluginsFile().resolve("fake") +
//                "] already exists; " +
...
//            @Override
//            Path downloadZip(Terminal terminal, String urlString, Path tmpDir, boolean isBatch)
//                throws IOException {
...
//                        "verify signature should not be called for unofficial plugins");
//                }
//            }
//
//            @Override
...
//        final String pluginId, final String name, final String url, final String stagingHash,
//        boolean isSnapshot) throws Exception {
//        final MessageDigest digest = MessageDigest.getInstance("SHA-512");
//        assertInstallPluginFromUrl(
//            pluginId, name, url, stagingHash, isSnapshot, ".sha512",
//            checksumAndFilename(digest, url), newSecretKey(), this::signature);
//    }
//
//    public void testOfficialPlugin() throws Exception {
//        String url =
//            "https://artifacts.elastic.co/downloads/elasticsearch-plugins/analysis-icu/analysis-icu-" +
//                Version.CURRENT + ".zip";
...
//        String url = "https://staging.elastic.co/" + Version.CURRENT +
...
//        String url = "https://repo1.maven.org/maven2/mygroup/myplugin/1.0.0/myplugin-" +
//            Platforms.PLATFORM_NAME + "-1.0.0.zip";
//        assertInstallPluginFromUrl("mygroup:myplugin:1.0.0", "myplugin", url, null, false);
//    }
//
...
//        assertInstallPluginFromUrl("mygroup:myplugin:1.0.0", "myplugin", url, null, false, ".sha1",
//            checksum(digest), null, (b, p) -> null);
//        assertTrue(terminal.getOutput(),
//            terminal.getOutput().contains("sha512 not found, falling back to sha1"));
//    }
//
...
//        MessageDigest digest = MessageDigest.getInstance("SHA-1");
//        UserException e = expectThrows(UserException.class, () ->
//            assertInstallPluginFromUrl("analysis-icu", "analysis-icu", url, null, false, ".sha1",
//                checksum(digest), null, (b, p) -> null));
...
//        UserException e = expectThrows(UserException.class, () ->
//            assertInstallPluginFromUrl(
//                "mygroup:myplugin:1.0.0", "myplugin", url, null, false, ".dne", bytes -> null, null,
//                (b, p) -> null));
//        assertEquals(ExitCodes.IO_ERROR, e.exitCode);
...
//        UserException e = expectThrows(UserException.class,
//            () -> assertInstallPluginFromUrl(
//                "analysis-icu", "analysis-icu", url, null, false, ".sha512", checksum(digest), null,
//                (b, p) -> null));
//        assertEquals(ExitCodes.IO_ERROR, e.exitCode);
...
//        UserException e = expectThrows(UserException.class, () ->
//            assertInstallPluginFromUrl(
//                "mygroup:myplugin:1.0.0", "myplugin", url, null, false, ".sha1", bytes -> "foobar",
//                null, (b, p) -> null));
//        assertEquals(ExitCodes.IO_ERROR, e.exitCode);
...
//        final BiFunction<byte[], PGPSecretKey, String> signature = (b, p) -> signature(b,
//            signingKey);
//        final PGPSecretKey verifyingKey = newSecretKey(); // the expected key used for signing
//        final String expectedID = Long.toHexString(verifyingKey.getKeyID()).toUpperCase(
//            Locale.ROOT);
//        final IllegalStateException e = expectThrows(
//            IllegalStateException.class,
//            () ->
//                assertInstallPluginFromUrl(
//                    icu, icu, url, null, false, ".sha512", checksumAndFilename(digest, url),
//                    verifyingKey, signature));
//        assertThat(e, hasToString(containsString(
//            "key id [" + actualID + "] does not match expected key id [" + expectedID + "]")));
//    }
//
...
//                assertInstallPluginFromUrl(
//                    icu, icu, url, null, false, ".sha512", checksumAndFilename(digest, url),
//                    newSecretKey(), signature));
//        assertThat(e, hasToString(equalTo(
//            "java.lang.IllegalStateException: signature verification for [" + url + "] failed")));
//    }
//
//    public PGPSecretKey newSecretKey()
...
//            null,
//            null,
//            new JcaPGPContentSignerBuilder(pkp.getPublicKey().getAlgorithm(),
...
//    }
//
//    private Function<byte[], String> checksumAndFilename(final MessageDigest digest,
...
//    private void assertPolicyConfirmation(Tuple<Path, Environment> env, String pluginZip,
